...
UI = os.path.join(os.environ.get("STDUIPATH"), "new_shelf_btn.ui")
...
class NewShelfBtn(QtGui.QDialog):
    def __init__(self, current_shelf_name, parent=None):
        ...
        self.setupEditor()
        # add drop in event
        python_drop_in = std_qt.dropable(self.code_edit_py)
        mel_drop_in = std_qt.dropable(self.code_edit_mel)
        icon_drop_in = std_qt.dropable(self.icon_label)
    ...
    def setupEditor(self):
        ...
        self.highlighter_py = std_qt.syntax.PythonHighlighter(self.code_edit_py.document())
        self.highlighter_mel = std_qt.syntax.MelHighlighter(self.code_edit_mel.document())
